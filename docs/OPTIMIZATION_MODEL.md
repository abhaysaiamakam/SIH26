...
## Independent validator (`apps/api/src/validation`)

Never assume the optimizer is correct merely because CP-SAT returned a solution. `ValidatorService.validateRevision(planRevisionId)` re-derives every constraint **from scratch**, in TypeScript, from Prisma-loaded state - it shares no code with `services/optimizer`, including its own from-scratch reimplementation of the compatibility rule precedence (`validation/compatibility-check.ts`). It checks: task uniqueness, block containment within its window, duration sufficiency, resource-overlap capacity (across every pair of blocks in the revision, not just adjacent ones), isolation/power permission, department permission, bundle work-type compatibility, dependency ordering, and hard-conflict train overlap. It returns `VALID` or `INVALID` with a typed `violations` array (`code`, `severity`, `message`, `relatedTaskIds`) and persists a `ValidationRun` row. `PlanningRunsService` and `WhatIfService` both call this immediately after persisting a plan and set `Plan.status` to `VALIDATED` or `INVALID` accordingly - **only a `VALIDATED` plan may be approved** (enforced server-side in `PlansService.approve`).

## What-If engine (`apps/api/src/what-if`)

Runs synchronously (these scenarios solve in well under a second): builds a "before" `OptimizerRunInput` from the unmodified scenario and runs/persists/validates/simulates it, applies the requested event, then does the same for an "after" input, and returns a `{ before, after, delta }` summary. Two event types are purely hypothetical, in-memory transforms of that one comparison's input (nothing is persisted to the database beyond the `ScenarioEvent` audit row): `CORRIDOR_UNAVAILABLE` (drops that corridor's block windows from the "after" input) and `BLOCK_WINDOW_SHORTENED` (shrinks one window's duration/end time) and `TASK_BECOMES_OVERDUE` (moves one task's due date into the past). Two event types represent genuinely new demand and are persisted as real rows before rebuilding the "after" input: `NEW_CRITICAL_REQUEST` (creates a real `MaintenanceRequest` with `CRITICAL`/`CRITICAL` criticality/urgency) and `ADDITIONAL_TRAIN_MOVEMENT` (creates a real `TrainMovement`). Every event, regardless of type, is recorded as a `ScenarioEvent` row with `source="WHAT_IF"` for audit history.
